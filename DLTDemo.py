...
df_rules = spark.createDataFrame(
    [
        ("VendorID","VendorID IS NOT NULL","validity"),  # create your data here, be consistent in the types.
        ("PULocationID", "PULocationID is not null","validity"),
    ],
...
dlt.apply_changes(
  ...
  source = "gold_trips",
  keys = ["VendorID"],
  sequence_by = col("gold_datetime"),
  #apply_as_deletes = expr("operation = 'DELETE'"),
  #except_column_list = ["operation", "sequenceNum"],
  stored_as_scd_type = "2",
  track_history_except_column_list = ["gold_datetime"]
)
